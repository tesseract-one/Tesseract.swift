...
use tesseract_utils::ptr::{SyncPtrAsVoid, SyncPtr, SyncPtrAsType};
...
impl tesseract_protocol_test::TestService for TestService {
  async fn sign_transaction(self: Arc<Self>, req: &str) -> tesseract::Result<String> {
      ...
      let future = future.try_into_future().map_err(|err| tesseract::Error::nested(Box::new(err)))?;
      let allow = future.await.map_err(|err| tesseract::Error::nested(Box::new(err)))?;
