pub use tesseract_utils::error::CError;
...
impl IntoCError for Error {
  fn into_cerror(self) -> CError {
    match self.kind {
      ErrorKind::Serialization =>  CError::ErrorCode(CErrorCodes::Serialization as u32, self.to_string().into()),
      ErrorKind::Cancelled => CError::Canceled,
      ErrorKind::Weird => CError::ErrorCode(CErrorCodes::Nested as u32, self.to_string().into())
    }
  }
}
